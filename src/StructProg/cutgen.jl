...
function gencut(::FeasibilityCutGenerator, sp::SOI.AbstractStochasticProgram, parent, pool::SOI.AbstractSolutionPool, stats, ztol)
    for tr in SOI.get(sp, SOI.OutTransitions(), parent)
        if SOI.hassolution(pool, tr)
            ...
            if SOI.getstatus(trsol) == :Infeasible
                stats.nfcuts += 1
                stats.fcutstime += SOI.@_time SOI.addcut!(sp, tr, FeasibilityCut(SOI.feasibility_cut(trsol)...))
            end
        end
    end
end
...
function gencut(::NoOptimalityCutGenerator, sp::SOI.AbstractStochasticProgram, parent, pool::SOI.AbstractSolutionPool, stats, ztol)
end
...
function gencut(::MultiCutGenerator, sp::SOI.AbstractStochasticProgram, parent, pool::SOI.AbstractSolutionPool, stats, ztol)
    for tr in SOI.get(sp, SOI.OutTransitions(), parent)
        if SOI.hassolution(pool, tr)
            trsol = SOI.getsolution(pool, tr)
            ...
            if status != :Unbounded
                ...
                if SOI.getstatus(sol) == :Unbounded || _lt(θ, β - dot(aT, x), ztol)
                    stats.ocutstime += SOI.@_time SOI.addcut!(sp, tr, MultiOptimalityCut(a, β))
                    stats.nocuts += 1
                end
            end
        end
...
function gencut(::AvgCutGenerator, sp::SOI.AbstractStochasticProgram, parent, pool::SOI.AbstractSolutionPool, stats, ztol)
    ...
    if SOI.getstatus(sol) == :Unbounded || _lt(θ, avgβ - dot(avga, x), ztol)
        stats.ocutstime += SOI.@_time SOI.addcut!(sp, parent, AveragedOptimalityCut(avga, avgβ))
        stats.nocuts += 1
    end
end
